...
#category_index = out['fc8_allweb'].argmax() #-if you want to get the saliency map of predicted class or else you can get saliency map for any interested class by specifying here

n = 5
top_labels = np.argsort(out['prob'])[0][(-1 * n):]

for category_index in top_labels:
    ...
    print("Gradients for %s Calculated" % categories[category_index])
